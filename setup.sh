#!/bin/bash

# BrightSign Python CV Extension - Setup Script
# Automates Steps 0-1: Prerequisites, clone, and source download

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Configuration
BRIGHTSIGN_OS_MAJOR_VERSION=${BRIGHTSIGN_OS_MAJOR_VERSION:-9.1}
BRIGHTSIGN_OS_MINOR_VERSION=${BRIGHTSIGN_OS_MINOR_VERSION:-52}
BRIGHTSIGN_OS_VERSION=${BRIGHTSIGN_OS_MAJOR_VERSION}.${BRIGHTSIGN_OS_MINOR_VERSION}

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

log() {
    echo -e "${BLUE}[$(date +'%T')] $1${NC}"
}

warn() {
    echo -e "${YELLOW}⚠️  $1${NC}"
}

error() {
    echo -e "${RED}❌ $1${NC}"
    exit 1
}

success() {
    echo -e "${GREEN}✅ $1${NC}"
}

# Prerequisites check
check_prerequisites() {
    log "Checking prerequisites..."
    
    # Check architecture
    if [[ "$(uname -m)" != "x86_64" ]]; then
        error "This build requires x86_64 architecture. Current: $(uname -m)"
    fi
    
    # Check Docker
    if ! command -v docker &> /dev/null; then
        error "Docker is required but not installed. Please install Docker first."
    fi
    
    # Check Docker daemon
    if ! docker info &> /dev/null; then
        error "Docker daemon is not running. Please start Docker."
    fi
    
    # Check required tools
    for tool in wget tar git; do
        if ! command -v $tool &> /dev/null; then
            error "$tool is required but not installed."
        fi
    done
    
    # Check disk space (need ~25GB)
    available_space=$(df . | awk 'NR==2 {print $4}')
    required_space=$((25 * 1024 * 1024)) # 25GB in KB
    
    if [[ $available_space -lt $required_space ]]; then
        warn "Warning: Less than 25GB free space available. Build may fail."
        warn "Available: $(($available_space / 1024 / 1024))GB, Recommended: 25GB+"
    fi
    
    success "Prerequisites check passed"
}

# Download BrightSign OS source
download_source() {
    log "Downloading BrightSign OS source files..."
    
    if [[ -d "brightsign-oe" ]]; then
        warn "brightsign-oe directory already exists. Removing..."
        rm -rf brightsign-oe
    fi
    
    # Download source archives
    log "Downloading source archives (~2-3 GB)..."
    wget -c "https://brightsignbiz.s3.amazonaws.com/firmware/opensource/${BRIGHTSIGN_OS_MAJOR_VERSION}/${BRIGHTSIGN_OS_VERSION}/brightsign-${BRIGHTSIGN_OS_VERSION}-src-dl.tar.gz" || \
        error "Failed to download dl archive"
    
    wget -c "https://brightsignbiz.s3.amazonaws.com/firmware/opensource/${BRIGHTSIGN_OS_MAJOR_VERSION}/${BRIGHTSIGN_OS_VERSION}/brightsign-${BRIGHTSIGN_OS_VERSION}-src-oe.tar.gz" || \
        error "Failed to download oe archive"
    
    # Extract archives
    log "Extracting source archives..."
    tar -xzf "brightsign-${BRIGHTSIGN_OS_VERSION}-src-dl.tar.gz" || error "Failed to extract dl archive"
    tar -xzf "brightsign-${BRIGHTSIGN_OS_VERSION}-src-oe.tar.gz" || error "Failed to extract oe archive"
    
    success "Source files downloaded and extracted"
}

# Setup Docker build environment
setup_docker() {
    log "Setting up Docker build environment..."
    
    # Download Dockerfile if not present
    if [[ ! -f "Dockerfile" ]]; then
        log "Downloading Dockerfile..."
        wget "https://raw.githubusercontent.com/brightsign/extension-template/refs/heads/main/Dockerfile" || \
            error "Failed to download Dockerfile"
    fi
    
    # Build Docker image
    log "Building Docker image (this may take 5-10 minutes)..."
    docker build --rm \
        --build-arg USER_ID=$(id -u) \
        --build-arg GROUP_ID=$(id -g) \
        --ulimit memlock=-1:-1 \
        -t bsoe-build . || error "Failed to build Docker image"
    
    # Create srv directory
    mkdir -p srv
    
    success "Docker environment ready"
}

# Main setup function
main() {
    echo "BrightSign Python CV Extension - Setup Script"
    echo "============================================="
    echo "This script will:"
    echo "1. Check prerequisites"
    echo "2. Download BrightSign OS source (~2-3 GB)"
    echo "3. Setup Docker build environment"
    echo ""
    
    # Confirm execution
    read -p "Continue? [y/N] " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Setup cancelled."
        exit 0
    fi
    
    start_time=$(date +%s)
    
    check_prerequisites
    download_source
    setup_docker
    
    end_time=$(date +%s)
    duration=$((end_time - start_time))
    
    echo ""
    success "Setup completed successfully in $(($duration / 60))m $(($duration % 60))s"
    echo ""
    echo "Next steps:"
    echo "1. Configure build: ./sh/patch-local-conf.sh -y"
    echo "2. Build SDK: ./patch-n-build.sh (30-90 minutes)"
    echo "3. Package extension: See README Step 3"
    echo ""
    echo "Environment variables set:"
    echo "export BRIGHTSIGN_OS_VERSION=${BRIGHTSIGN_OS_VERSION}"
    echo "export project_root=$(pwd)"
}

# Run main function if script is executed directly
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    main "$@"
fi